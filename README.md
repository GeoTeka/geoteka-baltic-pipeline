...
Sentinel-2 L2A scene
        |
        v
1. Load bands + scene mask (SCL)              src/io_s2.py
        |
        v
2. Sun glint correction (Hedley et al. 2005)  src/deglint.py
        |
        v
3. Water / land masking (NDWI)                src/classify.py
        |
        v
4. Vegetation classification (NDVI)           src/classify.py
        |
        v
5. Vectorise -> GeoJSON in EPSG:3006          src/classify.py
```

End-to-end run: [`notebooks/katthammarsvik_pipeline.ipynb`](notebooks/katthammarsvik_pipeline.ipynb).
...
jupyter lab notebooks/katthammarsvik_pipeline.ipynb
```

Point the notebook at a Sentinel-2 L2A scene covering Katthammarsvik (download from the
Copernicus Data Space Ecosystem; raster data is not committed, see `.gitignore`).
